...
import com.vaycore.finance.data.AGREEMENT_REGISTER
import com.vaycore.finance.data.PRIVACY_POLICY
import com.vaycore.finance.data.agreePhonePrivacy
...
//                launchPostLoginDestination()
                MainActivity.Companion.launch(this@LoginActivity)
                finish()
            }
        }
    ...
    private fun launchPostLoginDestination() {
        ...
        if (isFirstLogin) {
            SPUtil.newInstance().save(FirstSavingsPlanActivity.Companion.KEY_FIRST_LOGIN_HANDLED, true)
        }
    ...
    private fun hasHandledFirstLogin(): Boolean = SPUtil.newInstance().get(
        FirstSavingsPlanActivity.Companion.KEY_FIRST_LOGIN_HANDLED,
        false,
    )
